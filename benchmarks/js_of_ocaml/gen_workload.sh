#!/usr/bin/env bash
# gen_workload.sh — generate bytecode workload for js_of_ocaml benchmark.
#
# Compiles the OCaml stdlib to bytecode (.cma) which jsoo will then
# compile to JavaScript. This is the canonical jsoo workload — it
# exercises the compiler's code generation, optimization, and output.
#
# The generated workload is runtime-independent (bytecode is portable).
set -euo pipefail

BENCH_DIR="$(cd "$(dirname "$0")" && pwd)"
WORKLOAD="${BENCH_DIR}/workload.cmo"

if [ -f "${WORKLOAD}" ]; then
  echo "Workload already exists: ${WORKLOAD}"
  exit 0
fi

# Create a non-trivial OCaml program that exercises many stdlib features
cat > "${BENCH_DIR}/workload.ml" << 'OCAML'
...
echo "Compiling workload to bytecode..."
ocamlc -I +stdlib "${BENCH_DIR}/workload.ml" -o "${BENCH_DIR}/workload.byte"
# Extract just the .cmo for jsoo
ocamlc -c "${BENCH_DIR}/workload.ml" -o "${WORKLOAD}"
